...
class DiagramModel {
  ...
  addNode(
    ...
    // Vue.set(this._model.nodes, newNode.id, newNode)
  ...
  addPort(node_id, name, direction, type, subtype = "", logChange = true) {
    ...
    // Vue.set(this._model.ports, newPort.id, newPort)
  ...
  addLink(from, to, logChange = true) {
    ...
    if (fport && tport) {
      ...
      // Vue.set(this._model.links, newLink.id, newLink)
  ...
  findBy(type, id) {
    return this._model[`${type}s`][id.toString()];
  }
  findPort(id) {
    return this.findBy("port", id);
  }
  findNode(id) {
    return this.findBy("node", id);
  }
  findLink(id) {
    return this.findBy("link", id);
  }
  ...
  serialize() {
    return JSON.stringify(this._model, (key, val) => {
      return key.startsWith("_") ? null : val;
    });
  }
  ...
  deserialize(serializedModel) {
    ...
    this.node_count =
      Object.keys(this._model.nodes).reduce((i, max) => {
        return i > max ? i : max;
      }, -1) + 1;
    this.port_count =
      Object.keys(this._model.ports).reduce((i, max) => {
        return i > max ? i : max;
      }, -1) + 1;
    this.link_count =
      Object.keys(this._model.links).reduce((i, max) => {
        return i > max ? i : max;
      }, -1) + 1;
    this.undos = [];
    this.redos = [];
